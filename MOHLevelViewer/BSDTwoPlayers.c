...
void BSD2PReadEntryTableChunk(BSD2P_t *BSD,FILE *InFile)
{
    ...
    fseek(InFile,BSD2P_ENTRY_TABLE_POSITION + sizeof(BSDHeader_t),SEEK_SET);
...
void BSD2PReadRenderObjectChunk(BSD2P_t *BSD,FILE *InFile)
{
    ...
    fseek(InFile,BSD2P_ENTRY_TABLE_POSITION + sizeof(BSDHeader_t) + BSD2P_ENTRY_TABLE_SIZE,SEEK_SET);
...
void BSD2PReadNodeChunk(BSD2P_t *BSD,FILE *InFile)
{
    ...
    fseek(InFile,BSD->NodeOffset + sizeof(BSDHeader_t),SEEK_SET);
