...
int RenderObjectWriteBuffer(RenderObject_t *RenderObject,VRAM_t *VRAM,vec3 OutMin,vec3 OutMax)
{
    ...
    //TODO(Adriano): Buffer name should be connected with the currently exported RenderObject (e.g RenderObjectIDBuffer.bin)
    ...
    for (i = 0; i < RenderObject->NumFaces; i++) {
        ...
        U0 = (((float)CurrentFace->UV0.u + VRAMGetTexturePageX(VRAMPage)) / TextureWidth);
        V0 = (((float)CurrentFace->UV0.v + VRAMGetTexturePageY(VRAMPage, ColorMode)) / TextureHeight);
        
        fwrite(&U0, 1, sizeof(U0), OutBuffer);
        fwrite(&V0, 1, sizeof(V0), OutBuffer);
        
        R = CurrentFace->RGB0.r / 255.f;
        G = CurrentFace->RGB0.g / 255.f;
        B = CurrentFace->RGB0.b / 255.f;
        ...
        U1 = (((float)CurrentFace->UV1.u + VRAMGetTexturePageX(VRAMPage)) / TextureWidth);
        V1 = (((float)CurrentFace->UV1.v + VRAMGetTexturePageY(VRAMPage, ColorMode)) / TextureHeight);
        ...
        fwrite(&OutVector[1], 1, sizeof(OutVector[1]), OutBuffer);
        fwrite(&OutVector[2], 1, sizeof(OutVector[2]), OutBuffer);
        U2 = (((float)CurrentFace->UV2.u + VRAMGetTexturePageX(VRAMPage)) / TextureWidth);
        V2 = (((float)CurrentFace->UV2.v + VRAMGetTexturePageY(VRAMPage, ColorMode)) / TextureHeight);
    ...
    return (RenderObject->NumFaces * 3 * 8 * sizeof(float)) + (RenderObject->NumFaces * 3 * sizeof(unsigned short));

}
cgltf_buffer_view *RenderObjectAllocGlTFBufferViewXYZUVIndices(RenderObject_t *RenderObject,cgltf_buffer *Buffer)
{
    cgltf_buffer_view *Result;
    int NumVertices;
    int Stride;
    ...
    // First buffer contains vertex data
    ...
    Result[1].size = RenderObject->NumFaces * 3 * sizeof(unsigned short);
...
void RenderObjectExportCurrentAnimationToGlTF(RenderObject_t *RenderObject, VRAM_t *VRAM, const char *Directory, const char *EngineName)
{
    ...
    Data->asset.version = StringCopy("2.0");
    ...
    memset(&Data->nodes[0], 0, sizeof(cgltf_node));
    Data->nodes_count = 1;
    ...
    Data->samplers = (cgltf_sampler *)malloc(sizeof(cgltf_sampler));
    memset(&Data->samplers[0], 0, sizeof(cgltf_sampler));
    Data->samplers_count = 1;
    Data->samplers[0].mag_filter = cgltf_filter_type_linear;
    Data->samplers[0].min_filter = cgltf_filter_type_linear;
    Data->samplers[0].wrap_s = cgltf_wrap_mode_clamp_to_edge;
    Data->samplers[0].wrap_t = cgltf_wrap_mode_clamp_to_edge;
    
    Data->textures = (cgltf_texture *)malloc(sizeof(cgltf_texture));
    memset(&Data->textures[0], 0, sizeof(cgltf_texture));
    Data->textures_count = 1;
    
    Data->textures[0].image = &Data->images[0];
    Data->textures[0].sampler = &Data->samplers[0];
    ...
    Data->materials[0].has_pbr_metallic_roughness = true;
    Data->materials[0].pbr_metallic_roughness.base_color_texture.texture = &Data->textures[0];
    Data->materials[0].pbr_metallic_roughness.roughness_factor = 1.f;
    Data->materials[0].pbr_metallic_roughness.base_color_factor[0] = Data->materials[0].pbr_metallic_roughness.base_color_factor[1] = 
    ...
    memset(&Data->meshes[0].primitives[0], 0, sizeof(cgltf_primitive));
    ...
    asprintf(&Data->scene->name, "RenderObject%i", RenderObject->Id);
